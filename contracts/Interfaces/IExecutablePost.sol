...
//  Definitions:
- ExecutablePost = a social contract about future state of world, that is broadcasted to the world, and has monetary payoff linked to it.
- Executable Post struct = instance of struct that holds necessary information to fullfil the social contract
...
interface IExecutablePost {
    ...
    * @notice Step one in the Executable Post Lifecycle. This function:  
    ...
    - creates instance of struct holds all necessary information for future resolution
    ...
    * @param _lensPost Holds information about the post that is forwarded to Lens protocol. User/FrontEnd is responsible for structuring the _lensPost. 
    No infromation from _post is used in _lensPost automatically. For details see DataTypes.sol of the Lens protocol 
     */
    function post(
    ...
    /**
    * @notice One way close the lifecycle of Executable post. In this case the power to resolve lies solely in the competence of the "resolver". The resolver
    can split the value to 2 parties, so the sum of share is 100%
    This function:
        - tranfers share of ERC20 tokens to the beneficiaries
        - posts a comment to the original post on the behalf of resolver
        - marks the resolved field as true and closes the lifecycle
    * @notice reverts if these criteria are not met
        Criteria 1: The Executable Id must exist
        Criteria 2: You must use same resolution function as defined in original Executable Post struct
    ...
    * @param _resultPartyA How much of the amount get party A. Number from 0 to 100.
    * @param _resultPartyB How much of the amount get party B. Number from 0 to 100.
    * @param _commentData This is a message that gets forwarded to original Lens post as a comment. User/FrontEnd is responsible for structuring the _lensPost. 
    ...
    * @notice One way to close the lifecycle of Executable post. Resolved by result given by Oracle. 100% of value goes to either party A or Party B. 
    This function:
        - calls the oracle specified in the DataTypes.Post
        - based on the answer decides who won
        - tranfers all ERC20 tokens to the winning party 
        - posts a comment to the original post on the behalf of resolver
        - marks the resolved field as true and closes the lifecycle
    * @notice reverts if these criteria are not met
        Criteria 1: The Executable Id must exist
